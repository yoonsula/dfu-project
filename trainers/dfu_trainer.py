...
from trainers.training_log import TrainingLogger
from trainers.training_log import collect_environment_info
from trainers.training_log import count_model_parameters
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train a DFU binary classifier (dfu vs other) on frozen DINOv3 features.",
    )
    ...
    parser.add_argument(
        ...
        help="ImageFolder root with train/val and dfu/other class folders.",
    )
    parser.add_argument(
        "--head-type",
        type=str,
    ...
    parser.add_argument("--best-metric", type=str, choices=("accuracy", "f1"), default="f1")
    return parser
...
def make_dfu_lr_scheduler(
    ...
    if args.lr_scheduler != "cosine":
        raise ValueError("Only --lr-scheduler cosine or none is supported.")
...
def train(args: argparse.Namespace) -> None:
    ...
    backbone = DINOv3Backbone(model_path=args.dinov3_model, freeze=True).to(device)
    head = DFUFeatureClassifierHead(
        feature_dim=384,
        hidden_dim=args.hidden_dim,
    ...
    print(
        f"profile: head={args.head_type} lr={args.lr} "
