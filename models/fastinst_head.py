...
class FastInstSegHead(nn.Module):
    """Lightweight FastInst-style binary segmentation head."""
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        kernels = self.kernel_mlp(query)
        instance_logits = torch.einsum("bqk,bkhw->bqhw", kernels, pixel_features)
        instance_weight = torch.sigmoid(self.query_score(query)).view(
            batch_size,
            self.num_queries,
            1,
            1,
        ...
        logits = weighted_instance_logits.max(dim=1, keepdim=True).values
